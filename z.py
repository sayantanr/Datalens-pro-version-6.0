...
@st.cache_data
def load_data(file):
    """Load CSV, Excel, XML, JSON, or SQL files into a pandas DataFrame with caching.
    - CSV:   pd.read_csv
    - Excel: pd.read_excel
    - JSON:  pd.read_json
    - XML:   pd.read_xml (requires lxml)
    - SQL:   Execute script in in‑memory SQLite and load the first table.
    """
    try:
        ...
        if fname.endswith('.csv'):
            ...
            # XML parsing – requires pandas >= 1.3 and lxml installed
            try:
                df = pd.read_xml(file)
            ...
            # Load SQL script into in‑memory SQLite and read the first table
            import sqlite3
            ...
            try:
                ...
                if tables.empty:
                    st.error("SQL script did not create any tables.")
                    return None
            ...
            st.error("Unsupported file type. Upload CSV, Excel, JSON, XML, or SQL.")
            return None
        return df
    except Exception as e:
...
def generate_metrics_kpis_okrs(df: pd.DataFrame):
    """Generate a large list of synthetic metrics, KPI cards and OKR rows."""
    metrics = []
    kpis = []
    okrs = []
    # Basic metric definitions for each numeric column
    numeric_cols = [c for c in df.select_dtypes(include='number').columns]
    for col in numeric_cols:
        metrics.append({"Metric": f"total_{col}", "Column": col, "Type": "Sum"})
        metrics.append({"Metric": f"avg_{col}", "Column": col, "Type": "Mean"})
        metrics.append({"Metric": f"max_{col}", "Column": col, "Type": "Maximum"})
        metrics.append({"Metric": f"min_{col}", "Column": col, "Type": "Minimum"})
        
        total = df[col].sum()
        avg = df[col].mean()
        kpis.append({"KPI": f"Total {col}", "Formula": f"SUM({col})", "Value": f"{total:,.2f}"})
        kpis.append({"KPI": f"Average {col}", "Formula": f"AVG({col})", "Value": f"{avg:,.2f}"})
        
        okrs.append({"Objective": f"Improve {col} performance", "Key Result": f"Increase total {col} by 10%", "Target": f"{total * 1.10:,.2f}"})
        okrs.append({"Objective": f"Stabilize {col} variability", "Key Result": f"Reduce std dev of {col} by 5%", "Target": "(calculate)"})
    
    # Pad to 100+ entries using combinations of categorical columns
    cat_cols = [c for c in df.select_dtypes(exclude='number').columns]
    extra_idx = 0
    while len(metrics) < 120:
        if not cat_cols:
            break
        col = cat_cols[extra_idx % len(cat_cols)]
        metrics.append({"Metric": f"unique_{col}_{extra_idx}", "Column": col, "Type": "CountDistinct"})
        extra_idx += 1
        
    return pd.DataFrame(metrics), pd.DataFrame(kpis), pd.DataFrame(okrs)
...
def generate_more_stats_dashboard(df, dimensions, measures, dates):
    ...
    with automl_tab:
        # existing AutoML UI remains unchanged
        pass

    # ---------------------------------------------------------------------
    # NEW: Metrics & KPI tab – grouped in a single tab for any data source
    # ---------------------------------------------------------------------
    metrics_tab, extra_tab = st.tabs(["📊 Metrics & KPIs", "🛠️ Extras"])
    with metrics_tab:
        if df is not None:
            metrics_df, kpi_df, okr_df = generate_metrics_kpis_okrs(df)
            st.subheader("Generated Metrics ({} rows)".format(len(metrics_df)))
            st.dataframe(metrics_df, use_container_width=True)
            st.subheader("KPIs")
            st.dataframe(kpi_df, use_container_width=True)
            st.subheader("OKRs")
            st.dataframe(okr_df, use_container_width=True)
            # Download options for KPIs & OKRs
            csv_kpi = kpi_df.to_csv(index=False).encode()
            csv_okr = okr_df.to_csv(index=False).encode()
            st.download_button("Download KPIs CSV", data=csv_kpi, file_name="kpis.csv", mime="text/csv")
            st.download_button("Download OKRs CSV", data=csv_okr, file_name="okrs.csv", mime="text/csv")
        else:
            st.info("Upload a data file to see generated metrics, KPIs and OKRs.")
    with extra_tab:
        st.write("Additional utilities can be placed here.")
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader("Choose CSV, Excel, JSON, XML, or SQL files",
                                         type=['csv', 'xlsx', 'xls', 'json', 'xml', 'sql'],
    ...
    if uploaded_files:
        ...
        st.info("👆 Upload a CSV, Excel, JSON, XML, or SQL file to start building dashboards")
        st.markdown("""
        ### How it works:
        1. **Upload** your data files - multiple files will be combined
